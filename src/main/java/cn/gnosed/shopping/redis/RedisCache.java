...
import cn.gnosed.shopping.util.JsonSerializer;
import cn.gnosed.shopping.util.SystemClock;
 ...
 * @Description: jedis工具类，存取键值对，判断是或存在
 */
@Component
public class RedisCache {
     ...
     * 默认失效时间5 分钟
     */
    public static final int DEFAULT_CACHE_SECONDS = DateTimeUtil.getSeconds(TimeUnit.SECONDS, FIVE);

    /**
     * 最少默认失效时间，1 分钟
     */
    public static final int LEAST_CACHE_TIME = 1;

    /**
     * 最多默认失效时间，5 分钟
     */
    public static final int MOST_CACHE_TIME = 5;

    /**
     * 默认失效时间毫秒 5 分钟
     */
    public static final long DEFAULT_CACHE_MILLIS = DateTimeUtil.getMillis(TimeUnit.MINUTES, FIVE);
    ...
    public Boolean expire(String key, int duration, TimeUnit timeUnit) {
        validateKeyParam(key);
        //时间转换成毫秒
        long millis = DateTimeUtil.getMillis(timeUnit, duration);
     ...
     * @param value
     */
    public Boolean setNeverExpires(String key, Object value) {
        validateParam(key, value);
        String result = redisClient.invoke(jedisPool, (jedis) -> {
                    String srtResult = jedis.set(key.getBytes(), JsonSerializer.serialize(value));
                    return srtResult;
     ...
     * @param key
     * @param value
     * @param nxxx  NX/XX，如果取NX，则只有当key不存在是才进行set，如果取XX，则只有当key已经存在时才进行set
     * @param expx  EX/PX，代表数据过期时间的单位，EX代表秒，PX代表毫秒
     * @param time  过期时间，单位是expx所代表的单位
     * @return SET 在设置操作成功完成时，返回 OK
     */
    public String set(String key, String value, String nxxx, String expx, long time) {
        validateParam(key, value);
        String result = redisClient.invoke(jedisPool, (jedis) -> {
                    String srtResult = jedis.set(key, value, nxxx, expx, time);
                    return srtResult;
                }
        );
        return result;
     ...
     * 注：常用与分布式锁
     *
     * @param script 参数是一段 Lua 5.1 脚本程序。脚本不必(也不应该)定义为一个 Lua 函数。
     * @param keys   从 EVAL 的第三个参数开始算起，表示在脚本中所用到的那些 Redis 键(key)，这些键名参数可以在 Lua 中通过全局变量 KEYS 数组，用 1 为基址的形式访问( KEYS[1] ， KEYS[2] ，以此类推)
     * @param args   附加参数，在 Lua 中通过全局变量 ARGV 数组访问，访问的形式和 KEYS 变量类似( ARGV[1] 、 ARGV[2] ，诸如此类)
     * @return 返回eval执行结果
     */
    public Object eval(String script, List<String> keys, List<String> args) {
        Object result = redisClient.invoke(jedisPool, (jedis) -> {
                    Object objResult = jedis.eval(script, keys, args);
                    return objResult;
                }
        );
        return result;
